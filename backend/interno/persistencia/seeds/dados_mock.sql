...
FROM
    usuarios
WHERE
    email = 'joao.silva@mindtrace.dev' ON CONFLICT (usuario_id) DO NOTHING;
...
FROM
    usuarios
WHERE
    email = 'ana.costa@mindtrace.dev' ON CONFLICT (usuario_id) DO NOTHING;
...
FROM
    usuarios
WHERE
    email = 'bruno.lima@mindtrace.dev' ON CONFLICT (usuario_id) DO NOTHING;
...
FROM
    usuarios
WHERE
    email = 'ano.costo@mindtrace.dev' ON CONFLICT (usuario_id) DO NOTHING;

-- -----------------------------------------------------------------------------
-- 4. Vinculação Profissional-Paciente (Many-to-Many)
-- -----------------------------------------------------------------------------
INSERT INTO
    profissional_paciente (profissional_id, paciente_id)
SELECT
    p.id,
    pac.id
FROM
...
WHERE
    up.email = 'joao.silva@mindtrace.dev'
    AND upac.email IN (
        'ana.costa@mindtrace.dev',
        'bruno.lima@mindtrace.dev'
...
WHERE
    up.email = 'joao.silva@mindtrace.dev'
    AND upac.email = 'ana.costa@mindtrace.dev' ON CONFLICT (token) DO NOTHING;
...
FROM
    ...
    JOIN usuarios up ON p.usuario_id = up.id
WHERE
    up.email = 'joao.silva@mindtrace.dev' ON CONFLICT (token) DO NOTHING;
...
WHERE
    u.email = 'ana.costa@mindtrace.dev';
...
WHERE
    u.email = 'bruno.lima@mindtrace.dev';
