...
type RegistroHumor struct {
	...
	NivelHumor       int16     `json:"nivel_humor" gorm:"not null;check:nivel >= 1 AND nivel <= 5"`
	HorasSono        int16     `json:"horas_sono" gorm:"not null;check:horas_sono >= 0 and horas_sono <= "`
	...
	AutoCuidado      []string  `json:"auto_cuidado" gorm:"not null"`
