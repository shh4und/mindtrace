    ...
    <form @submit.prevent="submitMood" class="space-y-8">
      <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        ...
        <div class="flex flex-col md:flex-row md:space-x-4 space-y-2 md:space-y-0">
          <input type="text" :value="currentDate" class="flex-1 text-lg font-medium text-gray-700 bg-gray-100 rounded-md p-2 text-center" readonly>
          <input type="text" :value="currentDay" class="flex-1 text-lg font-medium text-gray-700 bg-gray-100 rounded-md p-2 text-center" readonly>
        </div>
      </div>
      ...
      <div class="bg-rose-100 rounded-lg shadow-sm border border-gray-200 p-6">
        <h2 class="text-xl font-semibold text-gray-900 mb-3">Horas de Sono</h2>
        <div class="flex items-center space-x-4">
          <div class="w-full">
            <div class="mb-3">
              <div class="icon-row">
                <button v-for="step in sleepSteps" :key="step" type="button" class="icon-step" :class="{ 'active': step <= sleepHours }" @click="sleepHours = step">
      ...
      <div class="bg-amber-50 rounded-lg shadow-sm border border-gray-200 p-6">
        <h2 class="text-xl font-semibold text-gray-900 mb-3">Nível de Energia</h2>
        <div class="flex items-center space-x-4">
          <div class="w-full">
            <div class="mb-3">
              <div class="icon-row">
                <button v-for="step in energySteps" :key="step" type="button" class="icon-step" :class="{ 'active': step <= energyLevel }" @click="energyLevel = step">
                  <span class="step-emoji" aria-hidden="true">🔋</span>
                </button>
              </div>
            </div>
            <input type="range" min="0" :max="energyMax" step="1" v-model.number="energyLevel" class="w-full slider" />
            <div class="flex justify-between text-sm mt-2 text-gray-600">
              <span>Baixa</span>
              <span>Moderada</span>
              <span>Alta</span>
      ...
      <div class="bg-yellow-50 rounded-lg shadow-sm border border-gray-200 p-6">
        <h2 class="text-xl font-semibold text-gray-900 mb-3">Nível de Stress</h2>
        <div class="flex items-center space-x-4">
          <div class="w-full">
            <div class="mb-3">
              <div class="icon-row">
                <button v-for="step in stressSteps" :key="step" type="button" class="icon-step" :class="{ 'active': step <= stressLevel }" @click="stressLevel = step">
                  <span class="step-emoji" aria-hidden="true">😤</span>
                </button>
              </div>
            </div>
            <input type="range" min="0" :max="stressMax" step="1" v-model.number="stressLevel" class="w-full slider" />
            <div class="flex justify-between text-sm mt-2 text-gray-600">
              <span>Calmo</span>
              <span>Moderado</span>
              <span>Alto</span>
      ...
      <div class="bg-green-200 rounded-lg shadow-sm border border-gray-200 p-6">
        <h2 class="text-xl font-semibold text-gray-900 mb-4">Atividades de Autocuidado realizadas hoje</h2>
        <div class="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 gap-4">
          <label v-for="activity in selfCareActivities" :key="activity" class="flex items-center space-x-3 text-black custom-checkbox-label">
          ...
          <div class="col-span-2 md:col-span-1">
            <input type="text" v-model="otherActivity" placeholder="Outra atividade..." class="w-full px-4 py-2 rounded-lg border border-gray-300 focus:ring-2 focus:ring-white focus:border-emerald-500 outline-none transition-colors text-gray-900 placeholder-gray-500">
          </div>
        </div>
      </div>

      <div class="bg-gray-50 rounded-lg shadow-sm border border-gray-200 p-6">
        <h2 class="text-xl font-semibold text-gray-900 mb-4">Diário de Anotações (Opcional)</h2>
        <p class="text-sm text-black mb-4">Escreva sobre seus pensamentos, sentimentos ou o destaque do seu dia.</p>
  <textarea v-model="notes" rows="6" placeholder="O que marcou seu dia? O que você gostaria de registrar?" class="w-full p-4 rounded-lg border border-gray-300 focus:ring-2 focus:ring-white focus:border-emerald-500 outline-none transition-colors"></textarea>
      </div>

      <div class="flex justify-end">
  <button type="submit" class="w-full md:w-auto bg-emerald-600 hover:bg-emerald-700 text-white font-medium py-3 px-6 rounded-lg transition-colors duration-200 focus:ring-2 focus:ring-emerald-500 focus:ring-offset-2 outline-none">
          Registrar Humor
        </button>
      </div>
    </form>
...
const energySteps = Array.from({ length: energyMax + 1 }, (_, i) => i);
const stressSteps = Array.from({ length: stressMax + 1 }, (_, i) => i);
...
const stressSvg = `<svg width="48" height="48" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><rect x="2" y="2" width="20" height="20" rx="6" fill="#FFF1F2"/><path d="M8 8l8 8M16 8l-8 8" stroke="#FF687A" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/></svg>`;
...
const emotions = ['Alegre', 'Otimista', 'Energizado', 'Contente', 'Confiante', 'Calmo', 'Ansioso', 'Estressado', 'Irritado', 'Grato'];
const selfCareActivities = ['Nenhuma Atividade', 'Leitura', 'Exercício', 'Yoga', 'Meditação', 'Música', 'Hobbies'];
...
.icon-step {
  padding: 10px 6px;
  flex: 0 0 28px; /* fixed step width to avoid overflow */
}
