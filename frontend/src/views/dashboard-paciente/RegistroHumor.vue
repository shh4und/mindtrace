  ...
  <div class="flex-1 p-4 md:p-8 w-full">
    <header class="mb-8 text-center md:text-left">
      ...
      <p class="text-gray-600 mt-1">
        Preencha como você se sentiu hoje e registre suas observações.
      </p>
    </header>
    ...
    <form @submit.prevent="submit" class="space-y-8" novalidate>
      <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        ...
        <div class="flex flex-col md:flex-row md:space-x-4 space-y-2 md:space-y-0">
          <input
            type="text"
            :value="currentDate"
            class="flex-1 text-lg font-medium text-gray-700 bg-gray-100 rounded-md p-2 text-center"
            readonly
      ...
      <fieldset class="bg-rose-50 rounded-lg shadow-sm border border-gray-200 p-6">
        <legend class="text-xl font-semibold text-gray-900 mb-4 text-center w-full">
          Como você se sentiu hoje?
        </legend>
        <div
          class="flex flex-wrap gap-6 justify-center"
          role="radiogroup"
          aria-label="Selecione seu humor"
        >
          <button
            ...
            :class="[
              'mood-btn flex flex-col items-center justify-center p-3 rounded-full transition',
      ...
      <fieldset class="bg-rose-100 rounded-lg shadow-sm border border-gray-200 p-6">
        <legend class="text-xl font-semibold text-gray-900 mb-6 text-center w-full">
          Horas de Sono
        </legend>
        <div class="flex flex-col md:flex-row items-center md:space-x-4">
              ...
              <div class="icon-row" role="group" aria-label="Indicadores visuais de sono">
                ...
                >
                  <Emoji
                    ...
                    :size="22"
                    set="facebook"
                    class="step-emoji hidden md:block"
                  />
                </button>
              </div>
            </div>
          ...
          <div class="w-full md:w-36 text-center display-area mt-6 md:mt-0" aria-live="polite">
            <div class="display-emoji" aria-hidden="true">
              <Emoji :data="emojiIndex" :emoji="sleepConfig.emoji" :size="36" set="facebook" />
            </div>
      ...
      <fieldset class="bg-amber-50 rounded-lg shadow-sm border border-gray-200 p-6">
        ...
        <div class="flex flex-col md:flex-row items-center md:space-x-4">
          ...
          <div class="w-full md:w-36 text-center display-area mt-6 md:mt-0" aria-live="polite">
            <div class="display-emoji" aria-hidden="true">
              <Emoji :data="emojiIndex" :emoji="energyConfig.emoji" :size="36" set="facebook" />
            </div>
            <div class="font-semibold display-value">
              {{ energyLevel }} / {{ energyConfig.max }}
            </div>
          </div>
        </div>
      </fieldset>
      ...
      <fieldset class="bg-yellow-50 rounded-lg shadow-sm border border-gray-200 p-6">
        ...
        <div class="flex flex-col md:flex-row items-center md:space-x-4">
          ...
          <div class="w-full md:w-36 text-center display-area mt-6 md:mt-0" aria-live="polite">
            <div class="display-emoji" aria-hidden="true">
              <Emoji :data="emojiIndex" :emoji="stressConfig.emoji" :size="36" set="facebook" />
            </div>
            <div class="font-semibold display-value">
              {{ stressLevel }} / {{ stressConfig.max }}
            </div>
          </div>
        </div>
      </fieldset>
      ...
      <fieldset class="bg-green-200 rounded-lg shadow-sm border border-gray-200 p-6">
        <legend class="text-xl font-semibold text-gray-900 mb-6">
          Atividades de Autocuidado realizadas hoje
        </legend>
        <div class="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 gap-4">
          ...
          >
            ...
            />
            <span
              class="w-7 h-7 border-2 border-gray-300/70 rounded-lg bg-white inline-flex items-center justify-center transition-all duration-150 peer-checked:bg-emerald-500 peer-checked:border-emerald-500 peer-checked:-translate-y-0.5"
              aria-hidden="true"
            >
              <svg
                v-if="selectedActivities.includes(activity)"
                width="14"
                height="14"
                viewBox="0 0 24 24"
                fill="none"
      ...
      <fieldset class="bg-gray-50 rounded-lg shadow-sm border border-gray-200 p-6">
        ...
        <p class="text-sm text-black mb-4">
          Escreva sobre seus pensamentos, sentimentos ou o que marcou seu dia. Isso ajudará você e
          seu profissional a entenderem o contexto do seu bem-estar.
        </p>
