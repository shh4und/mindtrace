  ...
  <div>
    ...
    <section class="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
      <div class="bg-white p-6 rounded-lg shadow-md border border-gray-200">
        <h2 class="text-xl font-semibold text-gray-900 mb-4">Último Registro de Humor</h2>
        <div class="flex items-center space-x-4">
          <span class="text-5xl mb-auto">{{ userLastData.emoji }}</span>
          <div>
            <p class="text-3xl font-bold text-green-600">{{ userLastData.label }}</p>
            <p class="text-gray-500 text-sm">{{ new Date(userLastData.data).toLocaleDateString('pt-BR', {
              weekday:
                'long', day: 'numeric', month: 'long' }) }}</p>
            <p class="text-gray-700 text-sm">{{ userLastData.anotacao == "" ? "Nada foi anotado." : userLastData.anotacao }}</p>
          </div>
        </div>
      </div>
      ...
      <div class="bg-white p-6 rounded-lg shadow-md border border-gray-200">
        <h1>TO-DO</h1>
        <h2 class="text-xl font-semibold text-gray-900 mb-4">Alertas e Notificações</h2>
        <ul class="space-y-4 text-gray-700">
          <li class="flex items-center space-x-3 p-3 bg-red-50 rounded-md border border-red-200">
            ...
            <div>
              ...
              <p class="text-sm text-gray-600">Seu humor esteve baixo por 3 dias consecutivos.</p>
            </div>
          </li>
        </ul>
      </div>
    ...
    <section class="bg-white p-6 rounded-lg shadow-md border border-gray-200">
      ...
      <ul class="space-y-4">
        <li class="flex items-start space-x-4">
          <div class="shrink-0 w-10 h-10 bg-gray-200 rounded-full flex items-center justify-center text-gray-600">
          ...
          <div>
            <p class="font-medium text-gray-900">Questionário "Bem-Estar Semanal" respondido.</p>
